...
import com.grottworkshop.gwschips.recipientchip.IDrawableRecipientChip;
import com.grottworkshop.gwschips.recipientchip.InvisibleRecipientChip;
import com.grottworkshop.gwschips.recipientchip.VisibleRecipientChip;
...
public class RecipientEditTextView extends MultiAutoCompleteTextView implements
    ...
    private IDrawableRecipientChip mSelectedChip;
    ...
    // VisibleForTesting
    ArrayList<IDrawableRecipientChip> mTemporaryRecipients;

    private ArrayList<IDrawableRecipientChip> mRemovedSpans;
    ...
    /*package*/ IDrawableRecipientChip getLastChip() {
        IDrawableRecipientChip last = null;
        IDrawableRecipientChip[] chips = getSortedRecipients();
    ...
    public void onSelectionChanged(int start, int end) {
        ...
        IDrawableRecipientChip last = getLastChip();
    ...
    private void shrink() {
        ...
        if (mSelectedChip != null && contactId != RecipientEntry.INVALID_CONTACT
            ...
            if (mPendingChipsCount > 0) {
                ...
                IDrawableRecipientChip[] chips =
                        getSpannable().getSpans(start, end, IDrawableRecipientChip.class);
                if ((chips == null || chips.length == 0)) {
    ...
    private IDrawableRecipientChip constructChipSpan(RecipientEntry contact, boolean pressed,
        ...
        IDrawableRecipientChip recipientChip =
    ...
    private void checkChipWidths() {
        ...
        if (chips != null) {
            Rect bounds;
            for (IDrawableRecipientChip chip : chips) {
        ...
        synchronized (mPendingChips) {
            ...
            if (mTemporaryRecipients != null && mTemporaryRecipients.size() > 0
                ...
                if (hasFocus() || mTemporaryRecipients.size() < CHIP_LIMIT) {
                    ...
                    mIndividualReplacements.execute(new ArrayList<IDrawableRecipientChip>(
                            mTemporaryRecipients.subList(0, CHIP_LIMIT)));
                    if (mTemporaryRecipients.size() > CHIP_LIMIT) {
                        mTemporaryRecipients = new ArrayList<IDrawableRecipientChip>(
    ...
    void createReplacementChip(int tokenStart, int tokenEnd, Editable editable,
        ...
        if (entry != null) {
            IDrawableRecipientChip chip = null;
            try {
                if (!mNoChips) {
                    /*
    ...
    /* package */ void sanitizeBetween() {
        ...
        // Find the last chip.
        IDrawableRecipientChip[] recips = getSortedRecipients();
        if (recips != null && recips.length > 0) {
            IDrawableRecipientChip last = recips[recips.length - 1];
            IDrawableRecipientChip beforeLast = null;
            if (recips.length > 1) {
    ...
    private int getChipStart(IDrawableRecipientChip chip) {
        return getSpannable().getSpanStart(chip);
    }

    private int getChipEnd(IDrawableRecipientChip chip) {
    ...
    protected void performFiltering(CharSequence text, int keyCode) {
        ...
        if (enoughToFilter() && !isCompletedToken) {
            ...
            IDrawableRecipientChip[] chips = span.getSpans(start, end, IDrawableRecipientChip.class);
            if (chips != null && chips.length > 0) {
                dismissDropDown();
                return;
            }
    ...
    private void showAlternates(final IDrawableRecipientChip currentChip,
    ...
    private ListAdapter createAlternatesAdapter(IDrawableRecipientChip chip) {
    ...
    private ListAdapter createSingleAddressAdapter(IDrawableRecipientChip currentChip) {
    ...
    private IDrawableRecipientChip findChip(int offset) {
        IDrawableRecipientChip[] chips =
                getSpannable().getSpans(0, getText().length(), IDrawableRecipientChip.class);
        // Find the chip that contains this offset.
        for (int i = 0; i < chips.length; i++) {
            IDrawableRecipientChip chip = chips[i];
    ...
    private CharSequence createChip(RecipientEntry entry, boolean pressed) {
        ...
        if (!mNoChips) {
            try {
                IDrawableRecipientChip chip = constructChipSpan(entry, pressed,
    ...
    public IDrawableRecipientChip[] getSortedRecipients() {
        IDrawableRecipientChip[] recips = getSpannable()
                .getSpans(0, getText().length(), IDrawableRecipientChip.class);
        ArrayList<IDrawableRecipientChip> recipientsList = new ArrayList<IDrawableRecipientChip>(
                Arrays.asList(recips));
        final Spannable spannable = getSpannable();
        Collections.sort(recipientsList, new Comparator<IDrawableRecipientChip>() {

            @Override
            public int compare(IDrawableRecipientChip first, IDrawableRecipientChip second) {
    ...
    /* package */ void createMoreChip() {
        ...
        IDrawableRecipientChip[] recipients = getSortedRecipients();
    ...
    public void removeMoreChip() {
        if (mMoreChip != null) {
            ...
            if (mRemovedSpans != null && mRemovedSpans.size() > 0) {
                ...
                Editable editable = getText();
                for (IDrawableRecipientChip chip : mRemovedSpans) {
                    int chipStart;
                    int chipEnd;
                    String token;
     ...
     */
    private IDrawableRecipientChip selectChip(IDrawableRecipientChip currentChip) {
        if (shouldShowEditableText(currentChip)) {
            ...
            IDrawableRecipientChip newChip;
            try {
                if (mNoChips) {
                    return null;
                }
    ...
    private boolean shouldShowEditableText(IDrawableRecipientChip currentChip) {
        long contactId = currentChip.getContactId();
        return contactId == RecipientEntry.INVALID_CONTACT
                || (!isPhoneQuery() && contactId == RecipientEntry.GENERATED_CONTACT);
    }

    private void showAddress(final IDrawableRecipientChip currentChip, final ListPopupWindow popup,
                             int width) {
        if (!mAttachedToWindow) {
            return;
        }
     ...
     */
    private void unselectChip(IDrawableRecipientChip chip) {
        int start = getChipStart(chip);
        int end = getChipEnd(chip);
        Editable editable = getText();
        mSelectedChip = null;
     ...
     * @return boolean
     */
    private boolean isInDelete(IDrawableRecipientChip chip, int offset, float x, float y) {
     ...
     */
    // Visible for testing.
    /* package */void removeChip(IDrawableRecipientChip chip) {
        Spannable spannable = getSpannable();
     ...
     */
    // Visible for testing.
    /*package*/ void replaceChip(IDrawableRecipientChip chip, RecipientEntry entry) {
        boolean wasSelected = chip == mSelectedChip;
        if (wasSelected) {
            mSelectedChip = null;
        }
     ...
     */
    public void onClick(IDrawableRecipientChip chip, int offset, float x, float y) {
        if (chip.isSelected()) {
    ...
    private class RecipientTextWatcher implements TextWatcher {
        ...
        public void afterTextChanged(Editable s) {
            ...
            if (TextUtils.isEmpty(s)) {
                ...
                Spannable spannable = getSpannable();
                IDrawableRecipientChip[] chips = spannable.getSpans(0, getText().length(),
                        IDrawableRecipientChip.class);
                for (IDrawableRecipientChip chip : chips) {
                    spannable.removeSpan(chip);
                }
                if (mMoreChip != null) {
        ...
        public void onTextChanged(CharSequence s, int start, int before, int count) {
            ...
            if (before - count == 1) {
                ...
                IDrawableRecipientChip[] repl = getSpannable().getSpans(selStart, selStart,
                        IDrawableRecipientChip.class);
                if (repl.length > 0) {
    ...
    public boolean isGeneratedContact(IDrawableRecipientChip chip) {
    ...
    // Visible for testing.
    /* package */ArrayList<IDrawableRecipientChip> handlePaste() {
        String text = getText().toString();
        ...
        IDrawableRecipientChip findChip = null;
        ArrayList<IDrawableRecipientChip> created = new ArrayList<IDrawableRecipientChip>();
        if (tokenStart != 0) {
            ...
            if (tokenStart != originalTokenStart) {
                ...
                int tokenEnd;
                IDrawableRecipientChip createdChip;
    ...
    private class RecipientReplacementTask extends AsyncTask<Void, Void, Void> {
        private IDrawableRecipientChip createFreeChip(RecipientEntry entry) {
            try {
                if (mNoChips) {
                    return null;
                }
        ...
        protected void onPreExecute() {
            ...
            // replaced
            final List<IDrawableRecipientChip> originalRecipients =
                    new ArrayList<IDrawableRecipientChip>();
            final IDrawableRecipientChip[] existingChips = getSortedRecipients();
            ...
            final List<IDrawableRecipientChip> replacements =
                    new ArrayList<IDrawableRecipientChip>(originalRecipients.size());

            for (final IDrawableRecipientChip chip : originalRecipients) {
        ...
        protected Void doInBackground(Void... params) {
            ...
            // chip.
            final ArrayList<IDrawableRecipientChip> recipients =
                    new ArrayList<IDrawableRecipientChip>();
            IDrawableRecipientChip[] existingChips = getSortedRecipients();
            for (int i = 0; i < existingChips.length; i++) {
            ...
            RecipientAlternatesAdapter.getMatchingRecipients(getContext(), adapter, addresses,
                    adapter.getAccount(), new RecipientAlternatesAdapter.RecipientMatchCallback() {
                        ...
                        public void matchesFound(Map<String, RecipientEntry> entries) {
                            final ArrayList<IDrawableRecipientChip> replacements =
                                    new ArrayList<IDrawableRecipientChip>();
                            for (final IDrawableRecipientChip temp : recipients) {
                        ...
                        public void matchesNotFound(final Set<String> unfoundAddresses) {
                            final List<IDrawableRecipientChip> replacements =
                                    new ArrayList<IDrawableRecipientChip>(unfoundAddresses.size());
        ...
        private void processReplacements(final List<IDrawableRecipientChip> recipients,
                                         final List<IDrawableRecipientChip> replacements) {
            if (replacements != null && replacements.size() > 0) {
                final Runnable runnable = new Runnable() {
                    ...
                    public void run() {
                        ...
                        int i = 0;
                        for (final IDrawableRecipientChip chip : recipients) {
                            final IDrawableRecipientChip replacement = replacements.get(i);
                            if (replacement != null) {
    ...
    private class IndividualReplacementTask
        ...
        protected Void doInBackground(ArrayList<IDrawableRecipientChip>... params) {
            ...
            // If there is a match, replace that chip with the matching
            // chip.
            final ArrayList<IDrawableRecipientChip> originalRecipients = params[0];
            ArrayList<String> addresses = new ArrayList<String>();
            IDrawableRecipientChip chip;
            ...
            RecipientAlternatesAdapter.getMatchingRecipients(getContext(), adapter, addresses,
                    ...
                    new RecipientAlternatesAdapter.RecipientMatchCallback() {
                        ...
                        @Override
                        public void matchesFound(Map<String, RecipientEntry> entries) {
                            for (final IDrawableRecipientChip temp : originalRecipients) {
     ...
     */
    private void startDrag(IDrawableRecipientChip currentChip) {
    ...
    private final class RecipientChipShadow extends DragShadowBuilder {
        private final IDrawableRecipientChip mChip;

        public RecipientChipShadow(IDrawableRecipientChip chip) {
            mChip = chip;
        }
